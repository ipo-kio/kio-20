...
export class Trafficlights
{
   ...
   /**
    ...
    constructor(settings)
    {
        ...
        this._levelSettings.roadCountX = Helper.getParamIntDef(this.settings.roadCountX, this._levelSettings.roadCountX);
        this._levelSettings.roadCountY = Helper.getParamIntDef(this.settings.roadCountY, this._levelSettings.roadCountY);
        this._levelSettings.roadWidth = Helper.getParamIntDef(this.settings.roadWidth, this._levelSettings.roadWidth);
        this._levelSettings.roadLen = Helper.getParamIntDef(this.settings.roadLen, this._levelSettings.roadLen);

        this._levelSettings.startCrossId = Helper.getParamIntDefStr(this.settings.startCrossId, this._levelSettings.startCrossId);
        this._levelSettings.finishCrossId = Helper.getParamIntDefStr(this.settings.finishCrossId, this._levelSettings.finishCrossId);
        ...
        for (var crossId1 in this._levelSettings.svets) 
        {
            ...
            {
                if (!this._svetoDic.hasOwnProperty(this._levelSettings.svets[crossId1]))
                {
                    sveto = new Sveto();
    ...
    parameters() {
        ...
        */
        ...
        return [
            ...
            {
                name: '_waitCount',
                title: 'Ожиданий на светофорах:',
                ordering: 'minimize'
            },
            {
                name: '_stopCount',
                title: 'Остановок на светофорах:',
                ordering: 'minimize'
            },
            {
                name: '_pointCount',
    ...
    initInterface(domNode, preferred_width)
    {
        ...
        let w = 20 +  ((this._levelSettings.roadCountX - 1) * roadLen) + ((this._levelSettings.roadCountX ) * roadWidth) + this.CANVAS_DOP_W; // 900;
        let h = 20 +  ((this._levelSettings.roadCountY) * roadLen) + ((this._levelSettings.roadCountY + 1 ) * roadWidth) + this.CANVAS_DOP_H;//400;
        ...
        this._slider = new Slider(controlDiv, 0, 100, 45, this.kioapi.getResource('slider_p'), this.kioapi.getResource('slider_p'));
    ...
    goStepPlus(plusminus)
    {
        ...
        if(tik > this._currentResult._tikCount)
        {
            this._slider.value2 = tik;
        }
        else{
    ...
    {   
        ...
        for(i=0; i < this._fullTrackRoadIds.length; i++)
        {
            ...
            sveto = this.getSvetoByCrossId(trackNode._crossId2);

            if(sveto != null)
            {
                goSveto = new Object();
                goSveto._crossId = trackNode._crossId2;
                goSveto._svetsDic = {};

                rg = false;

                svetoCross = this.getCrossById(trackNode._crossId2);
                cross1 = this.getCrossById(trackNode._crossId1);
                svetoSide = this.getSvetoSide(cross1, svetoCross);

                for(k=0; k < this._currentSolution._tikCount; k++)
                {
                    pr = sveto.getMyProgForTik(svetoSide, k);

                    if(pr.indexOf(trackNode._direct) >= 0)
                    {
                        rg = true;
                    }
                    else{
                    ...
                    goSveto._svetsDic[k] = rg;
                }

                this._goSvetsArr.push(goSveto);
            }
            ...
            for(j=0; j < (ttD); j++)
            {
                        ...
                        if(nextNode._thisTikCount > 1)
                        {
                            ...
                            if (nextNode._thisTikCount == 3) {
                                for (k = 0; k < n; k++) {
                                    ...
                                    goStepW._waitTik = 1;
                                    goStepW._direct = trackNode._direct;
                                    goStepW.x = goStep.x;
                                    goStepW.y = goStep.y;
                ...
                goStep._direct = trackNode._direct;
    ...
    goNext2()
    {
        ...
        if(!this._goStopFlag)
        {
            if(this._goStepIndex < this._goArr.length)
            {
                ...
                {
                    //console.log('go tik =' + goStep._tik );

                    if(!this._goSliderNoValue2)
                    {
                        this._slider.value2 = goStep._tik - 1;
                    }
                    
                    let goSveto;

                    for(i=0; i < this._goSvetsArr.length; i++)
                    {
                        goSveto = this._goSvetsArr[i];

                        if(goSveto._svetsDic[goStep._tik - 1 ] == true)
                        {
                            colorRed = 'silver';
                            colorGreen = 'green';
                            colorYellow = 'silver'
                        ...
                        //let svetoShape = this._stageTop.getChildByName('cross_' + goSveto._crossId);

                        let svetoShape = this._crossShapeDic['cross_' + goSveto._crossId];

                        
                        //crossShape.graphics.beginFill(color);
                        //crossShape.graphics.drawRect(0, 0, 20, 20);
                        ...
                        svetoShape.graphics.beginFill(colorRed);
                        ...
                        //svetoShape = new createjs.Shape();
                        svetoShape.graphics.beginFill(colorYellow);
                        svetoShape.graphics.drawCircle(x, y, r);
                        svetoShape.graphics.endFill();
                        //svetoCont.addChild(svetoShape);
                        ...
                        //svetoShape = new createjs.Shape();
                        svetoShape.graphics.beginFill(colorGreen);
                        svetoShape.graphics.drawCircle(x, y, r);
                        svetoShape.graphics.endFill();
                        //svetoCont.addChild(svetoShape);
    ...
    prepareAll()
    {
        ...
        backGroundShape.graphics.drawRect(10, 10
        ...
        /*
        console.log('this._startCrossId=' + this._startCrossId);
        console.log('this._finishCrossId=' + this._finishCrossId);
        console.log('this._startRoadIdH=' + this._startRoadIdH);
        console.log('this._startRoadIdV=' + this._startRoadIdV);
        console.log('this._startRoadIdH2=' + this._startRoadIdH2);
        console.log('this._startRoadIdV2=' + this._startRoadIdV2);
        */
        ...
        {         
            ...
            if(point != null)
            {
                ...
                if (cross.x == this._levelSettings.roadCountX - 1) //-- крайний правый ряд
                {
                    if (cross.y == 0) //-- правый верхний угол
                    {
                        pointShape.graphics.moveTo(roadWidth / 4 - roadWidth, roadWidth / 4 * 3 + roadWidth)
                            .lineTo(roadWidth / 4 * 2 - roadWidth, roadWidth / 4 + roadWidth)
                            .lineTo(roadWidth / 4 * 3 - roadWidth, roadWidth / 4 * 3 + roadWidth)
                            .lineTo(roadWidth / 4 - roadWidth, roadWidth / 4 * 3 + roadWidth);
                    }
                    else if (cross.y == this._levelSettings.roadCountY) //-- правый нижний угол
                    {
                        pointShape.graphics.moveTo(roadWidth / 4 - roadWidth, roadWidth / 4 * 3 - roadWidth)
                            .lineTo(roadWidth / 4 * 2 - roadWidth, roadWidth / 4 - roadWidth)
                            .lineTo(roadWidth / 4 * 3 - roadWidth, roadWidth / 4 * 3 - roadWidth)
                ...
                else if (cross.x == 0) //-- крайний левый ряд
                {
                    if (cross.y == this._levelSettings.roadCountY) //-- левый нижний угол
                    {
                        pointShape.graphics.moveTo(roadWidth / 4 + roadWidth, roadWidth / 4 * 3 - roadWidth)
                            .lineTo(roadWidth / 4 * 2 + roadWidth, roadWidth / 4 - roadWidth)
                            .lineTo(roadWidth / 4 * 3 + roadWidth, roadWidth / 4 * 3 - roadWidth)
                    ...
                    else if(cross.y == 0) //-- верхний левый угол
                    {
                        pointShape.graphics.moveTo(roadWidth / 4 + roadWidth, roadWidth / 4 * 3 + roadWidth)
                            .lineTo(roadWidth / 4 * 2 + roadWidth, roadWidth / 4 + roadWidth)
                            .lineTo(roadWidth / 4 * 3 + roadWidth, roadWidth / 4 * 3 + roadWidth)
                ...
                else if (cross.y == this._levelSettings.roadCountY) //-- нижний ряд
                {
                    pointShape.graphics.moveTo(roadWidth / 4 + roadWidth, roadWidth / 4 * 3 - roadWidth)
                        .lineTo(roadWidth / 4 * 2 + roadWidth, roadWidth / 4 - roadWidth)
                        .lineTo(roadWidth / 4 * 3 + roadWidth, roadWidth / 4 * 3 - roadWidth)
            ...
            hit.graphics.beginFill("#000").drawRect(0, 0, roadWidth + 1, roadWidth + 1);
        ...
        for(i = 0; i < roadForCreateArr.length; i++)
        {
            ...
            this.createRoad(R.vh, R.roadId, R.crossId, R.crossIdNext, R.x, R.y, R.X, R.Y);
        }
        ...
        border.graphics.drawRect(10, 10,
        ...
        this._infoBoxConteiner .x = ((roadCountX) * roadLen) + (roadCountX * roadWidth) - 40 ;
        ...
        this._goShape.graphics.drawRoundRectComplex(0,0, this._carW, 10, 4, 4, 4, 4);
    ...
    roadDelOrSwitch(currentRoadId)
    {
        ...
        for(var roadIdKey in this._roadSelectedDic)
        {
            ...
            if((selectedRoad._crossId1 == currentCrossId || selectedRoad._crossId2 == currentCrossId) && (selectedRoad._id != currentRoadId))
            {
        ...
        if(Helper.dicLen(nextRoadDic) > 0)
        {
            ...
            this._trackRoadCrossDic[key] = this.getNextRoadIdForClick(key, excludedRoadId, nextRoadDic);
    ...
    createRoad(vh, roadId, crossId, crossIdNext, x, y, X, Y)
    {
        ...
        else
        {
            ...
            {
                if(roadShapeBot.absY != 0) //-- исключаем крайние границы
                {
                    roadShapeBot.graphics.moveTo(0, 0);
    ...
    getRoadById(roadId, logSource)
    {
        if(this._roadDic.hasOwnProperty(roadId))
        {
            ...
            console.log('Error! Road not found - id=' + roadId + " logSource=" + logSource);
            return null;
        }
    }
    ...
    {
        if (
            (startCrossId == road2._crossId1 || startCrossId == road2._crossId2)
        )
        {
            return true;
        }
    ...
    trackAdd(currentTrakNode)
    {
        ...
        let svetoSide;
        let sveto;
        let svetoCross = null, cross1, cross2;
        let progForTik = '';
        ...
        if (!this._isTrackFull)
        {
            ...
            if (sveto != null) 
            {
                ...
                progForTik = sveto.getMyProgForTik(svetoSide, currentTrakNode._fullTikCount);
            ...
            if(this._trackRoadCrossDic.hasOwnProperty(s))
            {
                ...
                if(this.isRoadSelected(nextRoadId))
                {
                    ...
                    if (this.isRoadNext(currentTrakNode._crossId2, roadNext) && currentRoad._id != roadNext._id) 
                    {
                        trackAdded = this.trackAddNextRoad(currentTrakNode, svetoCross, roadNext, progForTik, sveto, svetoSide);
    ...
    trackAddNextRoad(currentTrakNode, svetoCross, roadNext, progForTik, sveto, svetoSide)
    {
        ...
        if(canNext)
        {
            ...
            if (svetoCross != null) // в какую сторону от светофора едем
            {
                ...
                if (sveto != null &&  progForTik.indexOf(newDir) < 0) //-- светофор не пускает, ищем следующую программу
                {                               
                    do
                    {
                        i++;
                        tik++;
                        nextProg = sveto.getMyProgForTik(svetoSide, currentTrakNode._fullTikCount + (tik) - 1);
            ...
            {
                roadAdded = true;
                let nextTrackNode = new TrackNode(roadNext, currentTrakNode, tik, this._startCrossId, '');
    ...
    updateTracks(goToStart)
    {
            ...
            if(!this.isRoadBlocked(road._id))
            {
                ...
                if(isRoadInTrack)
                {
                    ...
                    if(roadShape.vh == 'h')
                    {
                        ...
                        {
                            //-- влево
                            roadShape.graphics.moveTo(roadLen - roadWidth , roadWidth/2).lineTo(roadLen - roadWidth/2, roadWidth/4).lineTo(roadLen - roadWidth/ 2, roadWidth/4 * 3).lineTo(roadLen - roadWidth, roadWidth/2);
                            roadShape.graphics.moveTo(roadLen - roadWidth/2 , roadWidth/2).lineTo(roadLen, roadWidth/2);
                        }
                        else{
                            //-- вправо
                            roadShape.graphics.moveTo(roadWidth/2, roadWidth/4).lineTo(roadWidth , roadWidth/2).lineTo(roadWidth/2, roadWidth/4 * 3).lineTo(roadWidth/2, roadWidth/4);
                            roadShape.graphics.moveTo(0 , roadWidth/2).lineTo(roadWidth/2, roadWidth/2);
                        }
                    }
                    else
                    {
                        ...
                        {
                            //-- вверх
                            roadShape.graphics.moveTo(roadWidth/4, roadLen - roadWidth/2).lineTo(roadWidth/2, roadLen - roadWidth).lineTo(roadWidth/4 * 3, roadLen - roadWidth/2).lineTo(roadWidth/4, roadLen - roadWidth/2);
                            roadShape.graphics.moveTo(roadWidth/2, roadLen).lineTo(roadWidth/2, roadLen - roadWidth/2);
                        }
                        else{
                            //-- вниз
                            roadShape.graphics.moveTo(roadWidth/4, roadWidth/2).lineTo(roadWidth/4 * 3, roadWidth/2).lineTo(roadWidth/2, roadWidth ).lineTo(roadWidth/4, roadWidth/2);
                            roadShape.graphics.moveTo(roadWidth/2, 0).lineTo(roadWidth/2, roadWidth/2);
                        }
                    }
                                ...
                                //if(this._fullTrackDic.hasOwnProperty(roadNext._id))
                                {
                                    ...
                                    if(nextTrackNode._road._vh == 'h')
                                    {
                                        if(nextTrackNode._direct == 'L')
                                        {
                                            ...
                                            if(trackNode._direct == 'T')
                                            {
                                                roadShape.graphics.moveTo(roadWidth/2, roadWidth/2)
                                                .curveTo(roadWidth/2, -roadWidth/2, 0, -roadWidth/2);
                                            }
                                            else if(trackNode._direct == 'B')
                                            {
                                                roadShape.graphics.moveTo(roadWidth/2, roadLen - roadWidth/2)
                                                .curveTo(roadWidth/2, roadLen + roadWidth/2, 0, roadLen + roadWidth/2);
                                            }
                                            else if(trackNode._direct == 'L')
                                            {
                                                roadShape.graphics.moveTo(roadWidth/2, roadWidth/2)
                                            ...
                                            {
                                                roadShape.graphics.moveTo(roadWidth/2, roadWidth/2)
                                                .curveTo(roadWidth/2, -roadWidth/2, roadWidth, -roadWidth/2);
                                            }
                                            else if(trackNode._direct == 'B')
                                            {
                                                roadShape.graphics.moveTo(roadWidth/2, roadLen - roadWidth/2)
                                                .curveTo(roadWidth/2, roadLen + roadWidth/2, roadWidth, roadLen + roadWidth/2);                                             
                                            }
                                            else if(trackNode._direct == 'R')
                                            {
                                                roadShape.graphics.moveTo(roadLen - roadWidth/2,roadWidth/2).lineTo(roadLen + roadWidth,roadWidth/2);
                                            }
                                         }
                                    }
                                    else
                                    {
                                        if(nextTrackNode._direct == 'T')
                                        {
                                            ...
                                            if(trackNode._direct == 'R')
                                            {
                                                roadShape.graphics.moveTo(roadLen - roadWidth/2, roadWidth/2)
                                                .curveTo(roadLen + roadWidth/2, roadWidth/2, roadLen + roadWidth/2, 0);
                                            }
                                            else if(trackNode._direct == 'L')
                                            {
                                                roadShape.graphics.moveTo(roadWidth/2, roadWidth/2)
                                                .curveTo(-roadWidth/2, roadWidth/2, -roadWidth/2, 0);
                                            }
                                            else if(trackNode._direct == 'T')
                                            {
                                                roadShape.graphics.moveTo(roadWidth/2, roadWidth/2)
                                            ...
                                            if(trackNode._direct == 'R')
                                            {
                                                roadShape.graphics.moveTo(roadLen, roadWidth/2)
                                                .curveTo(roadLen + roadWidth/2, roadWidth/2, roadLen + roadWidth/2, roadWidth);
                                            }
                                            else if(trackNode._direct == 'L')
                                            {
                                                roadShape.graphics.moveTo(roadWidth/2, roadWidth/2)
                                                .curveTo(-roadWidth/2, roadWidth/2, -roadWidth/2, roadWidth);                                             
                                            }
                                            else if(trackNode._direct == 'B')
                                            {
                                                roadShape.graphics.moveTo(roadWidth/2, roadLen - roadWidth/2)
        ...
        if(this._selectedCrossId != '' && this._selectedCrossId != this._finishCrossId)
        {
            //-- отрисовка отмеченного светофора
            let color;

            crossShape = this._stageTop.getChildByName('cross_' + this._selectedCrossId);
            crossShape.graphics.clear();

            crossShape.graphics.beginFill(createjs.Graphics.getRGB(239, 239, 127, 0.7));
            crossShape.graphics.drawRect(1, 1, roadWidth-2, roadWidth-2);
            crossShape.graphics.endFill();
...
*/
    ...
    saveCurrentSolution()
    {
        //console.log('saveCurrentSolution()');

        var jsonArr = this.solution();

        localStorage.setItem(this.id() +  '_current3', jsonArr);
